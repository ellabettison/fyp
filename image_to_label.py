...
class ImageToLabel:
    ...
    def __init__(self, config):
        ...
        self.seg_shape = (config.img_size, config.img_size, 1)
        self.depth_shape = (config.img_size, config.img_size, 1)
        self.conv_layers = 2
    ...
    def define_cnn(self):
        ...
        inp_seg = Input(shape=self.seg_shape)
        inp_depth = Input(shape=self.depth_shape)

        inputs_to_use = []

        if self.use_canon:
            inputs_to_use.append(inp)
        if self.use_seg:
            inputs_to_use.append(inp_seg)
        if self.use_depth:
            inputs_to_use.append(inp_depth)

        combined_imgs = Concatenate(axis=-1)(inputs_to_use)

        c = self.conv2d_layer(combined_imgs, self.filters, stride=1)
